...
class LoginModel
{
     ...
     * @param $set_remember_me_cookie mixed|null Marker for usage of remember-me cookie feature
     *
     * @return bool success state
     */
    public static function login(string $user_name, string $user_password, mixed $set_remember_me_cookie = null): bool
    {
        ...
        // checks if user exists, if login isn't blocked (due to failed logins) and if the password fits the hash
        $result = self::validateAndGetUser($user_name, $user_password);

        // Check if that user exists. We don't give back a cause in the feedback to avoid giving an attacker details.
        if (!$result) {
            //No Need to give feedback here since whole validateAndGetUser controls gives feedback
            return false;
        }

        // stop the user's login if an account has been softly deleted
        if ($result->user_deleted == 1) {
            Session::add('feedback_negative', Text::get('FEEDBACK_DELETED'));
            return false;
        }

        // stop the user from logging in if user has a suspension, display how long they've left in the feedback.
        ...
        // maybe do this independence of setSuccessfulLoginIntoSession?
        return true;
    }
     ...
     * @return bool|mixed
     */
    private static function validateAndGetUser($user_name, $user_password): mixed
    {
        // brute force attack mitigation: use session failed login count and last failed login for not found users.
        // block a login attempt if somebody has already failed 3 times and the last login attempt is less than 30 sec ago
        // (limits user searches in a database)
        ...
        if (!$result) {
            ...
            // user doesn't exist, but we want to give a potential attacker this details, so we just use a basic feedback messenger
        ...
        // if hash of provided password doesn't match the hash in the database: +1 failed-login counter
        ...
        // if a user isn't active (= hasn't verified an account by verification mail)
     ...
     */
    private static function resetUserNotFoundCounter(): void
    {
     ...
     */
    private static function incrementUserNotFoundCounter(): void
    {
    ...
    public static function loginWithCookie(string $cookie): bool
    {
        ...
        // if user with that id and exactly that cookie token exists in a database
        if ($result) {
     ...
     */
    public static function setSuccessfulLoginIntoSession($user_id, $user_name, $user_email, $user_account_type): void
    {
        Session::init();

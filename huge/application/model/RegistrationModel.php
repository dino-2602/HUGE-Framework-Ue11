...
class RegistrationModel
{
     ...
     * third party services, like facebook) and creates a new user in the database if everything's fine
     *
    ...
    public static function registerNewUser(): bool
    {
        ...
        // get user_id of the user that's been created, to keep things clean we DON'T use lastInsertId() here
     ...
     * @return bool
     */
    public static function registrationInputValidation($user_name, $user_password_new, $user_password_repeat, $user_email, $user_email_repeat): bool
    {

        // if username, email and password are all correctly validated, but make sure they all run on the first summit
        if (self::validateUserName($user_name) AND self::validateUserEmail($user_email, $user_email_repeat) AND self::validateUserPassword($user_password_new, $user_password_repeat) AND true) {
            return true;
        }
     ...
     * @return bool
     */
    public static function validateUserName($user_name): bool
    {
        if (empty($user_name)) {
        ...
        // if username is too short (2), too long (64) or doesn't fit the pattern (aZ09)
     ...
     * @return bool
     */
    public static function validateUserEmail($user_email, $user_email_repeat): bool
    {
        if (empty($user_email)) {
     ...
     * @return bool
     */
    public static function validateUserPassword ($user_password_new, $user_password_repeat): bool
    {
     ...
     * @return bool
     */
    public static function writeNewUserToDatabase($user_name, $user_password_hash, $user_email, $user_creation_timestamp): bool
    {
        $database = DatabaseFactory::getFactory()->getConnection();

        // write new users data into a database
    ...
    /**
     * Deletes the user from users' table. Currently used to roll back a registration when verification mail sending
     * wasn't successful.
     *
     * @param $user_id
     */
    public static function rollbackRegistrationByUserId($user_id): void
    {
        $database = DatabaseFactory::getFactory()->getConnection();
     ...
     *
     * @return boolean gives back true if mail has been sent, gives back false if no mail could be sent
     */
    public static function sendVerificationEmail(int $user_id, string $user_email, string $user_activation_hash): bool
    {
        ...
        $mail = new Mail;
        return $mail->sendMail($user_email, Config::get('EMAIL_VERIFICATION_FROM_EMAIL'),
